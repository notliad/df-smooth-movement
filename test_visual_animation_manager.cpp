...
viewport_visual_animation_input make_input(
	const void *viewport,
	int32_t dimension,
	const int32_t *empty)
{
...
int main()
{
	...
	{
	constexpr int32_t width=5,height=3;
	int32_t empty[width*height]={};
	int32_t start[width*height]={};
	int32_t first[width*height]={};
	int32_t second[width*height]={};
	const int token=0;
	start[1*height+2]=77;
	first[2*height+1]=77;
	second[2*height+0]=77;
	visual_animation_manager chained;
	auto input=make_input(&token,width,empty);
	input.dim_y=height;
	set_layer(input,viewport_visual_layer::center,start,empty);
	run_frame(chained,input,1000);
	set_layer(input,viewport_visual_layer::center,first,start);
	run_frame(chained,input,1010);
	set_layer(input,viewport_visual_layer::center,second,first);
	run_frame(chained,input,1060);
	const auto movement=chained.get_movement(&token,viewport_visual_layer::center,2,0);
	assert(movement.active);
	assert(movement.source_x==1.5f&&movement.source_y==1.5f);
	assert(movement.progress==0.0f);
	assert(chained.get_facing(&token,2,0)==visual_facing::east);
	assert(chained.get_facing(&token,2,1)==native_sprite_facing);
	}
	...
	assert(get_visual_layer_descriptor(viewport_visual_layer::right).center_x==-1);
	assert(get_visual_layer_descriptor(viewport_visual_layer::left).center_x==1);
	assert(get_visual_layer_descriptor(viewport_visual_layer::upright).center_x==-1&&
		get_visual_layer_descriptor(viewport_visual_layer::upright).center_y==1);
	assert(get_visual_layer_descriptor(viewport_visual_layer::up).center_x==0&&
		get_visual_layer_descriptor(viewport_visual_layer::up).center_y==1);
	assert(get_visual_layer_descriptor(viewport_visual_layer::upleft).center_x==1&&
		get_visual_layer_descriptor(viewport_visual_layer::upleft).center_y==1);
