...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/stratum.XXXXXX")
...
mkdir -p "$work_dir/dfhack/plugins/external/stratum"
git -C "$repo_dir" archive "$plugin_tag" CMakeLists.txt stratum.cpp \
	...
	tar -x -C "$work_dir/dfhack/plugins/external/stratum"

cmake -S "$work_dir/dfhack" -B "$work_dir/dfhack/build/linux" -G Ninja \
	-DCMAKE_BUILD_TYPE=Release -DBUILD_DOCS=OFF
cmake --build "$work_dir/dfhack/build/linux" --target stratum
...
install -m755 "$work_dir/dfhack/build/linux/plugins/external/stratum/stratum.plug.so" "$work_dir/release/linux/hack/plugins/"
install -m755 "$work_dir/dfhack/build/win64-cross/output/hack/plugins/stratum.plug.dll" "$work_dir/release/windows/hack/plugins/"
(cd "$work_dir/release/linux" && zip -q -r "$output_dir/stratum-$release_tag-linux-x86_64.zip" hack)
(cd "$work_dir/release/windows" && zip -q -r "$output_dir/stratum-$release_tag-windows-x86_64.zip" hack)
for archive in "$output_dir"/stratum-"$release_tag"-*.zip; do unzip -t "$archive" >/dev/null; done
...
gh release upload "$plugin_tag" --repo "$repo" \
	"$output_dir/stratum-$release_tag"-*.zip --clobber
